...
* The PNG library used, PNGenc, have a bug on the calculation of crc of the IDAT chunck for big files when used on ESP32. We chose to fix this crc issue onboard with an image patcher so that the PNG files generated strictly match to the PNG specification. We did not dig far into PNGenc to see wether the bug was easy to fix or not as checking and patching individually each image was in fact easy and fast.
